...
export class InitUserAuthRefreshTokens1756489630504 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`refresh_tokens\` (\`id\` varchar(36) NOT NULL, \`token_hashed\` varchar(255) NOT NULL, \`user_agent\` json NOT NULL, \`ip_address\` varchar(255) NOT NULL, \`revoked\` tinyint NOT NULL DEFAULT 0, \`expires_at\` timestamp(0) NOT NULL, \`created_at\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, \`user_id\` varchar(36) NOT NULL, INDEX \`IDX_refresh_tokens_user_id\` (\`user_id\`), INDEX \`IDX_refresh_token_hashed\` (\`token_hashed\`), PRIMARY KEY (\`id\`), CONSTRAINT \`FK_3ddc983c5f7bcf132fd8732c3f4\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION) ENGINE=InnoDB`,
    );
  }
